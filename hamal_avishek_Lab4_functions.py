...
class SmartRaster(arcpy.Raster):
    ...
    def calculate_ndvi(self, band4_index=4, band3_index=3):
        ...
        # Your code:
        try:
            if arcpy.Exists(self.raster_path):  # check if the raster exists in workspace
                nir = arcpy.Raster(f"{self.raster_path}\\Band_{band4_index}")  # extract nir band (band 4)
                red = arcpy.Raster(f"{self.raster_path}\\Band_{band3_index}")  # extract red band (band 3)
            else:  # if the raster does not exist in the workspace
                okay = False
                returnval = f"Raster {self.raster_path} does not exist in workspace: {arcpy.env.workspace}"
                return okay, returnval
        except Exception as e:  # if the code in try block fails
            okay = False
            returnval = e
            return okay, returnval      
...
class SmartVectorLayer:
    ...
    # Add a field to store the zonal stats result.
        try:
            if arcpy.Exists(self.feature_class):
                existing_fields = [f.name for f in arcpy.ListFields(self.feature_class)]
                if output_field in existing_fields:
                    print(f"The field {output_field} already exists in the feature class.")
                    return False
                else:
                    arcpy.management.AddField(self.feature_class, output_field, "DOUBLE")
                    print(f"Added field {output_field} to the feature class.")
        except Exception as e:
            print(f"Problem adding field: {e}")
            return False
    # Create a temporary table to hold zonal statistics
        ...
        try:
            ...
            print(f"Zonal stats written to {temp_table}")
        except Exception as e:
            print(f"Problem calculating zonal stats: {e}")
            return False

    # Read through the Zonal stats table
        zonal_results = {}
        try:
            table_count = 0
    ...
    # Update the feature class with zonal results
            ...
            with arcpy.da.UpdateCursor(self.feature_class, ["OBJECTID", output_field]) as cursor:
                for row in cursor:
                    ...
                    if oid in zonal_results:
                        ...
                        print(f"Updated feature class with zonal stats in '{output_field}' field.")
        except Exception as e:
    ...
    def save_as(self, output_path):
        ...
        try:
            ...
            print(f"Problem saving feature class: {e}")
            return False
